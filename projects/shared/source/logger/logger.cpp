	...
	namespace shared
	{
			...
			try
			{
				for (const auto & attribute : attributes)
				{
					if (attribute.first != Attribute_Index::severity)
					{
						...
						{
							throw logger_exception("attribute [" + attribute.second.first + "] not added");
						}
					}
				}
			}
		...
		void Logger::add_sinks()
		{
			try
			{
				...
				core_ptr->add_sink(make_fout_sink());
			}
			catch (const std::exception & exception)
			{
				catch_handler < logger_exception > (FUNCTION, exception);
		...
		boost::shared_ptr < Logger::sink_t > Logger::make_fout_sink()
		{
			try
			{
				...
				// fout_sink_ptr->locked_backend()->scan_for_files();
		...
		void Logger::fout_formatter(const boost::log::record_view & record_view, boost::log::formatting_ostream & stream)
		{
			try
			{
				const boost::log::attribute_value_set & attribute_value_set =
					record_view.attribute_values();
		...
		const Logger::attributes_container_t Logger::attributes =
		{
			...
			{ Logger::Attribute_Index::severity, std::make_pair(
				  boost::log::aux::default_attribute_names::severity().string(), 
