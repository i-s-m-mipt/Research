...
namespace qlua {
  namespace table {
    LUA_TABLE_BEGIN(depo_limits_getDepo)
    LUA_TABLE_FIELD(depo_limit_locked_buy_value, double) // Стоимость ценных бумаг, заблокированных на покупку 
    LUA_TABLE_FIELD(depo_current_balance, double) // Текущий остаток по бумагам 
    LUA_TABLE_FIELD(depo_limit_locked_buy, double) // Количество лотов ценных бумаг, заблокированных на покупку 
    LUA_TABLE_FIELD(depo_limit_locked, double) // Заблокированное Количество лотов ценных бумаг 
    LUA_TABLE_FIELD(depo_limit_available, double) // Доступное количество ценных бумаг 
    LUA_TABLE_FIELD(depo_current_limit, double) // Текущий лимит по бумагам 
    LUA_TABLE_FIELD(depo_open_balance, double) // Входящий остаток по бумагам 
    LUA_TABLE_FIELD(depo_open_limit, double) // Входящий лимит по бумагам 
    LUA_TABLE_END()
  }
}
LUA_TABLE_TYPE_ADAPTER(::qlua::table::depo_limits_getDepo)

// money_limits_getMoney (table returned from getMoney API)
namespace qlua {
  namespace table {
    LUA_TABLE_BEGIN(money_limits_getMoney)
    LUA_TABLE_FIELD(money_open_limit, double) // Входящий лимит по денежным средствам 
    LUA_TABLE_FIELD(money_limit_locked_nonmarginal_value, double) // Стоимость немаржинальных бумаг в заявках на покупку 
    LUA_TABLE_FIELD(money_limit_locked, double) // Заблокированное в заявках на покупку количество денежных средств 
    LUA_TABLE_FIELD(money_open_balance, double) // Входящий остаток по денежным средствам 
    LUA_TABLE_FIELD(money_current_limit, double) // Текущий лимит по денежным средствам
    LUA_TABLE_FIELD(money_current_balance, double) // Текущий остаток по денежным средствам
    LUA_TABLE_FIELD(money_limit_available, double) // Доступное количество денежных средств
    LUA_TABLE_END()
  }
}
LUA_TABLE_TYPE_ADAPTER(::qlua::table::money_limits_getMoney)

/*
// level2_quotes_record
namespace qlua {
  namespace table {
    LUA_TABLE_BEGIN(level2_quotes_record)
    LUA_TABLE_FIELD(price, std::string)
    LUA_TABLE_FIELD(quantity, std::string)
    LUA_TABLE_END()
  }
}
LUA_TABLE_TYPE_ADAPTER(::qlua::table::level2_quotes_record)

// level2_quotes (table returned from getQuoteLevel2)
namespace qlua {
  namespace table {
    LUA_TABLE_BEGIN(level2_quotes)
    LUA_TABLE_FIELD(bid_count, std::string) // Количество котировок покупки. При отсутствии спроса возвращается значение «0» 
    LUA_TABLE_FIELD(offer_count, std::string) // Количество котировок продажи. При отсутствии предложения возвращается значение «0» 
    LUA_TABLE_FIELD(bid, ::std::vector<level2_quotes_record>) // Котировки спроса (покупки). При отсутствии спроса возвращается пустая строка 
    LUA_TABLE_FIELD(offer, ::std::vector<level2_quotes_record>) // Котировки предложений (продажи). При отсутствии предложения возвращается пустая строка 
    LUA_TABLE_END()
  }
}
LUA_TABLE_TYPE_ADAPTER(::qlua::table::level2_quotes)
*/
...
namespace qlua {
  namespace table {
    LUA_TABLE_BEGIN(current_trades_getParamEx)
    LUA_TABLE_FIELD(param_type, unsigned int) /* Тип данных параметра, используемый в Таблице текущих торгов. Возможные значения: 
                                              «1» - DOUBLE;
                                              «2» - LONG; 
                                              «3» - CHAR; 
                                              «4» - перечислимый тип; 
    ...
    LUA_TABLE_FIELD(param_value, double) // Значение параметра. Для param_type = 3 значение параметра равно «0», в остальных случаях – числовое представление. Для перечислимых типов значение равно порядковому значению перечисления  
    LUA_TABLE_FIELD(param_image, std::string) // Строковое значение параметра, аналогичное его представлению в таблице. В строковом представлении учитываются разделители разрядов, разделители целой и дробной части. Для перечислимых типов выводятся соответствующие им строковые значения  
    LUA_TABLE_FIELD(result, unsigned int) /* Результат выполнения операции. Возможные значения: 
                                          «0» - ошибка; 
                                          «1» - параметр найден */
    ...
    public ::lua::types::Table_Field<const char*, std::string> {
      using base_type = ::lua::types::Table_Field<const char*, std::string>;

      static auto get(lua::State s, int idx)  {
        return base_type::get(s, idx, "param_value");
      }

      static void apply(lua::State s, int idx, std::function<void(const lua::State&, int)> f) {
        base_type::apply(s, idx, f, "param_value");
      }

      static void set(lua::State s, int idx, const std::string& value)   {
        base_type::set(s, idx, value, "param_value");
      }
    };
    ::lua::Entity<param_value_as_string_type_policy> param_value_as_string{m_state, m_index};

    
    LUA_TABLE_END()
  }
}
LUA_TABLE_TYPE_ADAPTER(::qlua::table::current_trades_getParamEx)
